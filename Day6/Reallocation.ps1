...
#>
$ArrayMemoryBanks = @()
$HistoricalMemoryBanks = @()

$MemoryBanks = get-content "\\sync-al\files`$\JV-880707\tmcbrien\My Documents\PowershellScripts\AdventOfCode\Day6\Reallocation.txt"
$ArrayMemoryBanks = $MemoryBanks.split([char]0x0009)
foreach ($MemBank in $ArrayMemoryBanks) {
}

#Make Array to hold past values
#Compare current value with each past value