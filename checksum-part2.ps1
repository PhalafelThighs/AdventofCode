...
input
See : M:\My Documents\PowershellScripts\AdventOfCode\Day2\raw.txt
#>
$Arrayspreadsheet = @()
$splitrowSS = @()
$Total = 0

$Arrayspreadsheet = get-content "M:\My Documents\PowershellScripts\AdventOfCode\Day2\raw.txt"
foreach ($rowSS in $Arrayspreadsheet) {
    write-host "Row:     $rowSS"
    $splitrowSS = $rowSS.split([char]0x0009)
    foreach ($elementSSOuter in $splitrowSS) {
        foreach ($elementSSInner in $splitrowSS) {
            if(!($elementSSOuter%$elementSSInner) -and (!($elementSSOuter -eq $elementSSInner))) {
                $rowResult = ($elementSSOuter/$elementSSInner)
                $Total = $Total + ($elementSSOuter/$elementSSInner)
                write-host "$elementSSOuter / $elementSSInner = $rowResult"
            }
        }
    }
}
